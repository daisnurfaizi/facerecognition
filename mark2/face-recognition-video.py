...
#kuncipin = 11
#def buka(kuncipin):
#	GPIO.output(kuncipin,GPIO.HIGH)
#	return
#def tutup(kuncipin):
#	GPIO.output(kuncipin,GPIO.LOW)
#	return
#GPIO.setmode(GPIO.BOARD)
#GPIO.setup(kuncipin,GPIO.OUT)
   ...
   # Tampilkan kotak di wajah yang dideteksi
	...
	for encoding in encodings:
		matches = face_recognition.compare_faces(data["encodings"],
			encoding)
		name = "Tidak Dikenali"
		cv2.imwrite('opencv_stranger.jpg',frame)
		mm.tidak()
...
#			   buka(kuncipin)
			   time.sleep(5)
#			   tutup(kuncipin)
			if  name != "dais" and foto[0] != name:
				cv2.imwrite('opencv'+str(name)+'.jpg',frame)
				foto.insert(0,name)
				mm.kenal(name) 
		names.append(name)
	...
	cv2.imshow("Security face", frame)
